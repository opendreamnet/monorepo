...
class Logger {
  constructor(category, options) {
    ...
    let opts = {};
    Object.assign(opts, globalOptions);
  ...
  _write(level, text) {
    ...
    try {
      this.options.filename = this.options.filename || globalOptions.filename

      if (this.options.filename && !this.fileWriter && (isNodejs || isElectronRenderer)) {
  ...
  _format(level, text) {
    ...
    if (this.options.useColors) {
      ...
      const categoryColor = this.options.color || randomcolor({ seed: this.category, luminosity: 'dark' });
  ...
  _shouldLog(level) {
    ...
    const logLevel = envLogLevel || this.options.logLevel;
...
module.exports = {
  ...
  LogLevels,
  setLogLevel: (level) => {
    globalOptions.logLevel = level;
  },
  setLogfile: (filename) => {
    globalOptions.filename = filename;
  },
  setOptions: (options) => {
